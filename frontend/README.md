...
5. Go to `frontend/lib/managers/api_manager.dart` and replace the `BACKEND API URL HERE` with  `$NGROK_ADDRESS`.
...
9. In `main.dart`, change the line of code with `ApiManager.login(...)` to `ApiManager.login($YOURUSERNAME, $YOURPASSWORD)`.
